...
export async function GET() {
  try {
    ...
    const transformedCategories = categories.map(category => {
      ...
      if ((category.id.toString() === '1' || category.id.toString() === '11') && specialCategory) {
    ...
    return NextResponse.json({ error: 'Failed to fetch drink categories' }, { status: 500 });
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ ...category, id: category.id.toString() }, { status: 201 });
  } catch (error) {
    console.error('Error creating drink category:', error);
    return NextResponse.json({ error: 'Failed to create drink category' }, { status: 500 });
  }
}
